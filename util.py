...
class VkSession:
    def __init__(self, version: str, token: str, base="https://api.vk.com/method/"):
    ...
    def append(self, last: dict, current: dict):
        ...
        if len(d) > 0:
            now = str(datetime.datetime.utcnow() +
                      datetime.timedelta(hours=3))  # force GMT+3
            print(d)
            self.log[now] = d
...
class SpyEye:
    ...
    def gaze(self, user_id, logger: Logger, fields=fields):  # можно и домен и айди вроде
        self.logger = logger
        ...
        while True:
            ...
            self.current = self.session.exec(
                "users.get", user_id=self.user_id, fields=self.fields)[0]
